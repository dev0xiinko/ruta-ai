...
import { loadDebugRoutePageData } from "@/lib/server/debug-route-data";
...
export default async function DebugPage({ searchParams }: DebugPageProps) {
  ...
  const {
    counts: { variantCount, mapRefCount, stopCount, stopOrderCount, bindingCount },
    variants,
    selectedVariant,
    mapRefs,
    stopOrder,
    bindings,
  } = await loadDebugRoutePageData({
    codeFilter,
    selectedVariantKey,
  });
